...
import org.certificateservices.custom.c2x.groupsig.Bls12381PS16Signature;
...
public class DefaultCryptoManager implements Ieee1609Dot2CryptoManager {
	...
	public org.certificateservices.custom.c2x.ieee1609dot2.datastructs.basic.Signature signMessageDigest(
		...
		try{
			...
			// System.err.println("This is what is signed: " + Arrays.toString(digest));
			//System.out.println("digest: " + Hex.toHexString(digest));
			//System.out.println("Digest length: " + digest.length);

			com.ibm.jgroupsig.Signature digestSignature = groupUser.sign(digest, memKey);
			//System.out.println("signature length: " + digestSignature.toString().length());
			String digestSignatureString = digestSignature.export();

			// System.out.println("Generated digestSignatureBase64: " + digestSignatureString);

			//System.out.println("exported signature: " + digestSignatureString);
			//System.out.println("exported signature length: " + digestSignatureString.length());
			//System.out.println("public key: " + groupUser.getGrpKey().export());
			//System.out.println("public key length: " + groupUser.getGrpKey().export().length());

			Bls12381PS16Signature bls12381PS16Signature = new Bls12381PS16Signature(
					digestSignatureString,
					groupUser.getGrpKey().export()
			);

			// System.out.println("Generated BLS12381 PS16 signature: " + bls12381PS16Signature);

			return new org.certificateservices.custom.c2x.ieee1609dot2.datastructs.basic.Signature(
					SignatureChoices.bls12381PS16Signature,
					bls12381PS16Signature
			);
	...
	public org.certificateservices.custom.c2x.ieee1609dot2.datastructs.basic.Signature signMessage(
		...
		try{
			// System.out.println("Sign group signature with algorithm: " + alg.getAlgorithm().getSignature());
			return signMessageDigest(
					genIEEECertificateDigest(
							alg,
							message,
							null
					),
	...
	public boolean verifySignatureDigest(
		...
		try{
			if(signature.getType() != Signature.SignatureChoices.bls12381PS16Signature){
				throw new BadArgumentException("Expected bls12381PS16Signature but got: " + signature.getType());
			}
			Bls12381PS16Signature blsSignature = (Bls12381PS16Signature) signature.getValue();
			String tbvGroupSignatureExported = blsSignature.getGroupSig();

			com.ibm.jgroupsig.Signature tbvGroupSignature =
					new com.ibm.jgroupsig.Signature(
							GS.PS16_CODE,
							tbvGroupSignatureExported
					);
			// System.err.println("Verifying group signature: " + tbvGroupSignatureExported);
			// System.err.println("With message: " + Arrays.toString(digest));

			return groupUser.verify(tbvGroupSignature, digest);
		}catch(Exception e){
			if(e instanceof BadArgumentException){
				throw (BadArgumentException) e;
			}
	 ...
	 */
	@Override
	public byte[] genIEEECertificateDigest(
			AlgorithmIndicator alg,
			byte[] messageData,
			org.certificateservices.custom.c2x.ieee1609dot2.datastructs.cert.Certificate signerCertificate
	...
	protected boolean verifyGroupSignature(
			byte[] message,
			org.certificateservices.custom.c2x.ieee1609dot2.datastructs.basic.Signature signature,
			PS16 groupUser
	) throws BadArgumentException, SignatureException, IOException {
		...
		try{
			// System.out.println("Verifying group signature with algorithm: " + alg.getAlgorithm().getSignature());
			return verifySignatureDigest(
					genIEEECertificateDigest(
							SignatureChoices.bls12381PS16Signature,
							message,
							null
            ...
            switch (e) {
                case BadArgumentException badArgumentException -> throw badArgumentException;
                case IOException ioException -> throw ioException;
                case SignatureException signatureException -> throw signatureException;
                default -> {
                }
            }
