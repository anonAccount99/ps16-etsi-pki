...
public class SecuredDataGenerator {
	 ...
	 */
	public Ieee1609Dot2Data genSignedData(
			HeaderInfo hi,
			byte[] message,
	...
	public Ieee1609Dot2Data genSignedData(
		...
		assert signerIdentifierType != SignerIdentifierType.SELF;
		try{
			Ieee1609Dot2Data unsecuredData = new Ieee1609Dot2Data(
					version,
					new Ieee1609Dot2Content(
							Ieee1609Dot2ContentChoices.unsecuredData,
							new Opaque(message)
					)
			);

			ToBeSignedData tbsData = new ToBeSignedData(
					new SignedDataPayload(
							unsecuredData,
							null),
					hi
			);

			return genSignedDataStructure(
					tbsData,
					signerIdentifierType,
					groupUser,
					memKey,
	 ...
	 * @throws IOException if IO exception occurred communicating with underlying systems. 
	 */
	protected Ieee1609Dot2Data genSignedData(
			HeaderInfo hi,
			byte[] message,
			SignerIdentifierType signerIdentifierType,
			Certificate[] signerCertificateChain,
	 ...
	 * @throws IOException if IO exception occurred communicating with underlying systems. 
     */
	public static boolean verifySignedData(
	...
	public static boolean verifySignedData(
		...
		if(signedData.getContent().getType() != Ieee1609Dot2ContentChoices.groupSignedData){
			throw new BadArgumentException("Only groupSignedData can verified. Was of type: " + signedData.getContent().getType());
		}

		try{
			GroupSignedData sd = (GroupSignedData) signedData.getContent().getValue();
			if(sd.getTbsData().getPayload().getData() == null){
				throw new BadArgumentException("Error no enveloped data found in Signed Payload");
			}

			// Extract the signature from the GroupSignedData
			Signature sentSignature = sd.getSignature();

			// Extract the group signature as a string from the BLS signature
			if(sentSignature.getType() != Signature.SignatureChoices.bls12381PS16Signature){
				throw new BadArgumentException("Expected bls12381PS16Signature but got: " + sentSignature.getType());
			}

			// System.err.println("sd.getTbsData().getEncoded(): " + Arrays.toString(sd.getTbsData().getEncoded()));

			return cryptoManager.verifySignature(
					sd.getTbsData().getEncoded(),
					sentSignature,
					group
			);
		} catch (Exception e) {
	...
	public static DecryptResult decryptDataWithSecretKey(Ieee1609Dot2CryptoManager cryptoManager, Ieee1609Dot2Data encryptedData, Map<HashedId8, Receiver> recieverStore) throws BadArgumentException, GeneralSecurityException, IOException{
		...
		byte[] data = cryptoManager.symmetricDecryptIEEE1609_2_2017(
				symmetricCiphertext.getType(),
				getEncryptedData(symmetricCiphertext),
				decryptionKey.getEncoded(),
				getNounce(symmetricCiphertext)
		);
   ...
   public static byte[] getEncryptedData(SymmetricCiphertext symmetricCiphertext) {
	...
	protected Ieee1609Dot2Data genSignedDataStructure(
			...
			PrivateKey signerPrivateKey,
			boolean enveloped
	) throws BadArgumentException, InvalidKeySpecException, SignatureException, IOException, NoSuchAlgorithmException{
		SignerIdentifier si;
		...
		Signature signature = cryptoManager.signMessage(
				tbsData.getEncoded(),
				signAlgorithm,
				signerPrivateKey,
				certificateType,
	...
	protected Ieee1609Dot2Data genSignedDataStructure(
		...
		// System.err.println("tbsData.getEncoded() at sign time: " + Arrays.toString(tbsData.getEncoded()));
		...
		GroupSignedData groupSignedData = new GroupSignedData(
				hashAlgorithm,
				tbsData,
				signature
		);

		Ieee1609Dot2Content content = new Ieee1609Dot2Content(groupSignedData);
