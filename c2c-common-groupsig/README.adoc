...
----
	    ...
	    EtsiTs103097Certificate rootCACertificate = authorityCertGenerator.genRootCA("testrootca.test.com", // id
...
----
        	    ...
        	    Map<HashedId8, Certificate> certStore = securedMessageGenerator.buildCertStore(new EtsiTs103097Certificate[] {authorizationCert, authorityCACertificate});
        	      // To decrypt build a reciever store of known decryption keys and related DENMReceiver info, this can be certificate, signed message containing encryption key
...
----
        ...
        // Then create a DENMReceiver store to decrypt the message
